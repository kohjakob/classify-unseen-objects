...
class DataModel:
    ...
    def load_scannet_scene_data(self, scene_name):
        ...
        # UMAP's spectral initialization requires N > n_components + 1 samples.
        # With default n_components=2, we need N > 3, so at least 4 samples.
        if self.instance_features.shape[0] > 3:
            num_samples = self.instance_features.shape[0]

            # n_neighbors must be less than the number of samples.
            safe_n_neighbors = min(15, num_samples - 1)

            reducer = umap.UMAP(n_neighbors=safe_n_neighbors, random_state=42)
            self.umap_embedding = reducer.fit_transform(self.instance_features)
        else:
            # Handle cases where there aren't enough samples for UMAP
            print(f"Warning: Not enough instance features ({self.instance_features.shape[0]} samples) to compute UMAP. Needs >3. Setting embedding to None.")
            self.umap_embedding = None
        # --- END OF FIX ---

        # Set cluster selection state
        self.current_cluster = np.unique(self.instance_labels)[0] if len(self.instance_labels) > 0 else None
